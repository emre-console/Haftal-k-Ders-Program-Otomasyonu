...
namespace HaftalikDersProgrami
{
    public partial class ProgramOlusturucu : Form
    {
        ...
        LinkedList<ProgramPanel> panels = new LinkedList<ProgramPanel>();
        ...
        void drawProgram()
        {
            ...
            foreach(string s in Pazartesi)
            {
                ...
                saat = yenisaat;
                dakika = yenidakika;
                if(saat == int.Parse(oglenArasiBaslangic.Split('-')[0]) && dakika == int.Parse(oglenArasiBaslangic.Split('-')[1]))
                {
                    dakika = int.Parse(oglenArasiSaat);
                    while (true)
                    {
                        if (dakika >= 60)
                        {
